...
# set STATUS_COLOR: yellow for "insert", green for "normal" mode.
prompt_purer_vim_mode() {
	STATUS_COLOR="${${KEYMAP/vicmd/13}/(main|viins)/yellow}"
...
prompt_pure_preprompt_render() {
  ...
  if [[ "$PURER_PROMPT_COMMAND_COUNT" -gt "$FIRST_COMMAND_THRESHOLD" ]]; then
	...
	local symbol_color="%(?.${PURE_PROMPT_SYMBOL_COLOR:-14}.red)"

	# show virtual env
	preprompt+="%(12V.%F{242}%12v%f .)"
	# directory, colored by vim status
	preprompt+="%F{$STATUS_COLOR}$(prompt-pwd)"
	# git info
